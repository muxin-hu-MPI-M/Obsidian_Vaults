...
- However, in my case, since the velocity is replaced to the Lagrangian framework in the initialisation, and the tendency terms are now the Lagrangian tendencies, we might have a trouble in the drag coefficient
	- **Actually NO**, because in my ERA5-provider + Stokes run, the wind stress is prescribed from ERA5, not recomputed from the ocean surface velocity, The ERA5 provider receives `ustress` and `vstress` as separate coupling fields [mo_ocean_era5_provider_coupling.f90 (line 152)](/work/mh0033/m301254/proj_surfwave/icon-2026-06-ocean-era5/m301254/era5g-wave-forcing/src/coupling/mo_ocean_era5_provider_coupling.f90:152). These are written directly into `p_as%topBoundCond_windStress_u/v` through the provider call [mo_ocean_surface_refactor.f90 (line 997)](/work/mh0033/m301254/proj_surfwave/icon-2026-06-ocean-era5/m301254/era5g-wave-forcing/src/ocean/boundary/mo_ocean_surface_refactor.f90:997). Then, with the default `bulk_wind_stress_type = wind_stress_from_file = 0` [mo_ocean_nml.f90 (line 844)](/work/mh0033/m301254/proj_surfwave/icon-2026-06-ocean-era5/m301254/era5g-wave-forcing/src/ocean/config/mo_ocean_nml.f90:844), the code just copies those stresses into `atmos_fluxes%stress_xw/yw` [mo_ocean_bulk_forcing.f90 (line 1272)](/work/mh0033/m301254/proj_surfwave/icon-2026-06-ocean-era5/m301254/era5g-wave-forcing/src/ocean/boundary/mo_ocean_bulk_forcing.f90:1272).
	- So it is “updating” in time only when the ERA5 provider sends a new forcing field. If no new provider data are due, the code only advances the coupling field clock and keeps the current arrays [mo_ocean_era5_provider_coupling.f90 (line 365)](/work/mh0033/m301254/proj_surfwave/icon-2026-06-ocean-era5/m301254/era5g-wave-forcing/src/coupling/mo_ocean_era5_provider_coupling.f90:365). It is not recalculated every ocean timestep from `U10 - U_ocean`.
- Another issue might be that **in the wave-ocean coupling, the “wind stress” is only calculated as the “stress of air flow over sea waves”**, depends on the sea state and from a consideration of the momentum balance of air it is found that the l]kinematic stress is given as Eq. (26-27) in [[ECMWF_2024_Note_Wave-induced-stress_and_Young-waves#Stress of Air Flow]]: $$\tau_{\text{a}} = \rho_a u_* |u_*|,\qquad u_*^2=\bar\tau_a =\bigg(\frac{\kappa\mathbf{U}(z_{\text{obs}})}{\ln(\frac{z_{\text{obs}}+z_o}{{z_o}})}\bigg)^2$$
	- In this case, the square of air-side friction velocity (density-normalised stress of the air) depends on the (1) wind velocity at mean height above the waves, and (2) the wave stress $\tau_w$ it self, as the $z_o=z_b/\sqrt{1-\frac{\tau_w}{\tau_a}}$, while the wave stress comes from the wave model as: $\tau_w=\epsilon^{-1}g\int \gamma N(\omega, \theta) \mathbf{k}\;d\omega d\theta$
	- the actual **stress going into the ocean** is: $$\tau_{\text{oc}}=\tau_{\text{a}} - \tau_{\text{in}}-\tau_{\text{ds}}$$
	  where:
		- $\tau_a$ is the atmospheric stress (or stress of air flow over sea waves)
		- $\tau_{in}$ is the wind-to-wave stress, represents the momentum flux from thw ind that are used for generation of ocean waves
		- $\tau_{ds}$ is the dissipation stress at the surface that describes the dissipation of waves by processes at the air-sea interface
# [[2026-08-12]]
## Regular Meeting with Nils
### suggested pipeline
...
- should update the normal velocity, the one used in model, in the tendency of the velocity 
