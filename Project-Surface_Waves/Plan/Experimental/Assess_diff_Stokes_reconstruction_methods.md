...
- It uses the Phillips-spectrum profile of Breivik et al. \citep{Breivik2016}, which is a stronger one-dimensional approximation than the monochromatic profile and the earlier exponential-integral profile, especially for near-surface Stokes shear.
...
- It directly matches the main research question: whether adding a physically plausible bulk Stokes profile affects ICON at climate-relevant scales.

- It keeps the closure assumption explicit. The surface value and profile direction are prescribed by the ERA5 surface Stokes drift, while the estimated total-sea Stokes transport magnitude constrains the inverse depth scale.

- It treats the experiment as a response to a bulk one-dimensional Stokes forcing, rather than implying unsupported wind-sea or swell attribution.

- It avoids mixing physically different reconstruction regimes within one operational forcing product. There is no need to combine successfully decomposed profiles with fallback profiles that have a different physical meaning.

- It is easier for reviewers to evaluate because the limitation is clear: it is a one-dimensional bulk approximation to a directional wave problem. The method is approximate, but it is not internally ambiguous.
...
- It is a one-dimensional representation of a full two-dimensional wave spectrum. The method cannot retain the complete directional structure of the wave field.

- It prescribes the direction of the full reconstructed profile from the ERA5 surface Stokes drift. This is a known limitation because the surface Stokes drift is weighted toward shorter waves, whereas the transport and deeper profile may be more influenced by longer swell.

- It uses the magnitude of the estimated total-sea Stokes transport to set the profile depth scale, but the reconstructed profile itself is collinear with the surface Stokes drift. Thus, the method cannot preserve an independent transport direction.

- It cannot represent vertical directional rotation of the Stokes drift profile, which may occur when wind sea and swell propagate in different directions or nearly opposing directions.

- It does not distinguish whether the Stokes forcing arises mainly from wind sea or swell, and it should not be used to attribute the modeled response to individual wave systems.
...
- It uses the same Phillips-spectrum vertical profile as `STOKES_TOTAL`, so the difference between the methods lies in the input closure rather than in the vertical shape function.
...
- It is useful for diagnosing the closure problem itself: where the total surface Stokes vector can or cannot be represented by non-negative wind-sea and swell components.
...
- It could provide stronger physical interpretation in subsets of the ocean where the decomposition is admissible and where the response can be linked to well-defined wave regimes.

### Weaknesses

- It is not universally admissible. The decomposition works only where the total surface Stokes vector lies inside the positive cone spanned by the wind-sea and total-swell directions.
...
- It inherits uncertainty from partition-mean directions. These directions are bulk summaries and may not be weighted in the same way as surface Stokes drift, Stokes transport, or the shear-relevant part of the profile.

- It can look like a more physical method while still depending on an underdetermined closure. When the decomposition fails, the failure reflects an information problem in the bulk diagnostics, not necessarily a physically meaningful absence of either component.
