...
Here, the Coriolis term $-\mathbf f\times \mathbf u^L=-\mathbf f\times (\mathbf u^E+\mathbf u^s)$ introduces the Coriolis-Stokes forcing. The Stokes shear term ($-u_j^L\nabla u_j^s$), which primarily drives small-scale turbulence, is intentionally neglected. In the present formulation the Stokes–shear term is intentionally neglected. By including only the Lagrangian transport and the corresponding rotational adjustment, the model captures the minimal, dynamically consistent representation of the large-scale effect of Stokes drift.

To implement this mechanism in the model, prescribed Stokes drift fields derived from wave reanalysis products are incorporated. The effective Lagrangian velocity is used in the advection of mass, momentum, and tracers, while the Coriolis–Stokes forcing is included in the momentum equation. Because the Stokes drift is prescribed and does not interact dynamically with the wave field, the experiment isolates the circulation response to wave-driven transport without introducing the full complexity of wave–current interactions. Compensating Eulerian flows develop naturally, providing a physically meaningful adjustment of the large-scale circulation.
