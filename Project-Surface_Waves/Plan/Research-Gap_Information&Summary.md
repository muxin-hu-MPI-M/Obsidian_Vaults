...
Despite this growing recognition, the role of surface waves in coastal upwelling dynamics remains poorly quantified (Wu et al., 2019). Previous studies have demonstrated that surface waves can significantly influence air-sea interactions in coastal regions and corresponding upwelling dynamics. For example, using a regional Nucleus for European Modelling of the Ocean (NEMO) configuration forced by the WAve Model (WAM), Wu et al. (2019) showed that sea-state-dependent momentum and turbulent kinetic energy (TKE) fluxes exert a strong influence on ocean
