...
Despite this growing recognition, the role of surface waves in coastal upwelling dynamics remains poorly quantified (Wu et al., 2019). Previous studies have demonstrated that surface waves can significantly influence air-sea interactions in coastal regions and corresponding upwelling dynamics. For example, based on a wave hindcast generated by WAVEWATCH III forced with Climate Forecast System Reanalysis data, Wu et al. (2022) found that the redistributive effects of waves on momentum and TKE fluxes are significant in several major coastal upwelling regions around the globe. Similarly, using a regional Nucleus for European Modelling of the Ocean (NEMO) configuration forced by the WAve Model (WAM), Wu et al. (2019) showed that the wave-induced sea-state-dependent momentum and turbulent kinetic energy (TKE) fluxes exert a stronger influence on Baltic coastal upwelling than that from Stokes-Coriolis forcing and Stokes drift on the mass and tracer advection. These results thus indicate that various wave-induced processes can play distinct roles in shaping air-sea fluxes and upwelling dynamics, forming complex combined effect.

However, the interpretation of these results is constrained by methodological limitations. In particular, the usual modelling framework used are uncoupled or one-way coupled, preventing feedbacks between the atmosphere, wave field, and ocean interior. Moreover, not all relevant wave-effects are represented consistently. For example, Langmuir turbulence, an important contributor to near-surface TKE, is not always included in these configurations. Consequently, the existing studies provide insights into individual wave effects and their relative roles within specific model setups, they do not offer a dynamically consistent of the combined influence of surface waves on coastal upwelling systems. 
