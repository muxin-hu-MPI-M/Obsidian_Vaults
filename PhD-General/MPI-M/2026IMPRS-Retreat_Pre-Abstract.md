...
Together, these findings suggest that Stokes drift could matter way beyond the local. Yet its effects on resolved circulation and tracer transport are not routinely represented in models used for climate-scale studies. Its net influence on large-scale tracer redistribution, including heat, remains poorly quantified.

All of these leads to the central question of my work: Could wave-induced Stokes transport affect global ocean heat redistribution on the climate-relevant scales?

To make this question more concrete, also motivated by previous studies, I first estimate the scale and spatial structure of the Stokes transport based on ERA5 wave reanalysis. Here the map shows the global field of the Stokes mass/volume transport, represents the magnitude of the depth-integrated Stokes drift. We can again clearly observed its close relationships with the wind field. Then, based on the volume transport, I estimated the Stokes temperature transport by multiply it with the SST, under the assumption that the Stokes layer is within the near-surface layer with similar temperature as SST. The second panel here shows the zonal integration, expressed in PW of temperature transport, the PWT. The zonal integration plot illustrates that the Stokes temperature transport is spatially organised, with coherent equatorward transport in the subtropics, and convergent at the equator. This diagnose suggesting that may partly oppose the usual poleward redistribution of heat by large-scale circulations.

However, as you may already see in the slide title, I add the quotation marks in the heat transport, this is not the full story, simply due to the fact that the mass isn’t balance here in the simple diagnostics. 

Another very important part of the Story is the ocean adjustments. When Stokes drift is introduced, it will trigger the ocean adjustments, inducing the so-called Eulerian compensating flow. These adjustments can reshape the imposed Stokes drift/transport. This is because the ultimate velocity that transport mass and tracers is now the combination of Stokes drift with the Eulerian compensating flow. Under the wave-averaged theory, this ultimate transport velocity is the Wave-averaged Lagrangian velocity. We can clearly see how the Eulerian compensating flow reshape the initial Stokes drift

Now we go back to the previous question, we now realised that diagnosing the net heat transport doesn’t work. To quantify the net effect coming from the Stokes transport, we need to introduce the Stokes drift dynamically into ocean model to allow the ocean adjustments to develop freely.

So how can we do that? we need to modify the ICON-O primitive equations. Starting with the momentum equation. The original momentum equation here solves, or prognosing the Eulerian mean velocity, while the modified version here, which shares a lot common parts, is solving the Lagrangian velocity since we add the additional Stokes forcing here. By initialising the model with the Lagrangian velocity, and add the Stokes forcing, we can naturally prognosing the Lagrangian velocity. Using this framework, we can analyse the actual transport velocity, and the contributions from either Eulerian ocean current or the Stokes.
