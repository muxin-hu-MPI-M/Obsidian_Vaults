...
I then estimated the Stokes temperature transport that combines the volume transport with the sea surface temperature (SST),  assuming that water within the Stokes layer has approximately the same temperature as the surface. The second panel here shows its zonal integration, expressed in PettaWatts of temperature transport, the PWT. The zonal integration plot shows that the Stokes temperature transport is spatially organised, with coherent equatorward transport in the subtropics, leading to convergence at the equator. Comparing to ocean heat transport climatology, we find that our Stokes temperature transport is opposing the poleward heat transport induced by large-scale circulation.

However, as you may already noticed the quotation marks around heat transport in the title, these estimates capture only one part of the story. They omit the compensating volume transport and the temperatures carried by that flow, so they cannot determine the net heat transport alone.

The other part is the ocean’s dynamical adjustments. When Stokes drift is introduced, it will trigger the ocean Eulerian adjustments, inducing the Eulerian compensating flow, pressure-gradient changes, and altered vertical motion. These adjustments can reshape the influence of Stokes drift. This is because the change in transport velocity includes both the imposed Stokes drift and the resulting change in Eulerian circulation. And under the wave-averaged framework, water and tracers move with the Lagrangian velocity: the sum of Stokes drift and the Eulerian mean current. 

Returning to our research question, we now realised that diagnose only establish the potential transport. To quantify its **net effect**, we need to introduce the Stokes drift dynamically into an ocean model and allow the ocean adjustment to develop.

So how do we do this? we modify the ICON-O primitive equations. Starting with the momentum equation. The original formulation evolves the Eulerian mean velocity. Our modified formulation retains much of the existing structure, including the Coriolis, diffusion, and pressure-gradient terms, but includes two additional terms shown in red. These two terms are the Stokes forcing. With this introduction and consistent initialisation, the model then evolves the actual transport velocity, the Lagrangian velocity. Subtracting the prescribed Stokes drift gives the Eulerian current. With this framework, we can therefore examine the combined transport, distinguish its Stokes and Eulerian components, and assess changes relative to the reference simulation.

Now, the basic implementation is finished and has passed initial sanity and consistency checks. We have also completed paired five-year ICON-o simulations at R2B4 resolution. Both use ERA5 atmospheric forcing, while the experiment additionally includes Stokes forcing reconstructed from ERA5 wave fields. For this preliminary analysis, I focus on global northward ocean heat transport, or OHT, averaged over the final two years.

The figure shows the control mean, the OHT anomaly, and its convergence. Here, the anomaly is the experiment minus the control. Note that the control mean and anomaly use different vertical scales. The largest changes occur in the mid-latitudes of both hemispheres, reaching approximately 0.08 petawatts, or 80 terawatts. Across the equator, our Stokes-forced experiment shows a southward heat-transport anomaly of about 34 terawatts, equivalent to roughly 12% of the magnitude of the control mean. 

The lower panel shows the corresponding heat-transport convergence anomaly, expressed in the unit of heat flux. The response differs between hemispheres, with anomalous heat convergence in the Southern Ocean and divergence in the Northern Hemisphere mid-latitudes. In these regions, the magnitudes can reach to 5 W/m^2.

There are three points I would like you to take away:
- The net climate-scale influence of Stokes drift on ocean heat transport remains poorly quantified
- Stokes drift is now implemented in ICON-O, allowing the ocean to adjust dynamically and its net response to be assessed.
- Preliminary results show changes in the global OHT, motivating further assessment of their spatiotemporal patterns and persistence.

I’d like to end my presentation with some outlooks in the future. The current experiments prescribe Stokes drift from reanalysis and isolate one wave-induced pathway. Our longer-term aim is to use ICON-Wave in fully coupled atmosphere–wave–ocean simulations, allowing the wave field and circulation to interact and examining how different wave effects act together. My current implementation provides a foundation for representing Stokes-related ocean dynamics within that coupled framework.
...
- **Why could this matter for climate?**
	...
	- ~={red}gap: The question emerges: whether accounting this underrepresented wave-induced pathway in the ocean can leave a net imprint on large-scale heat redistribution.=~
- **Introduce Stokes drift into ICON-o**
	- To move from potential transport to net climate-scale imprint, we introduce Stokes drift into the ICON-O in a dynamically consistent way. 
		...
		- Once Stokes drift is included, the ocean adjust through compensating Eulerian currents, pressure-gradient changes, and altered vertical motion. These responses can oppose, reshape, or redistribute the impact of the imposed Stokes forcing.
...
- **Why could this matter for climate?**
	...
	- As Stokes drift also transport tracers like heat, it may contribute to heat redistribution. 
		- ==Figure 2(b) heat transport== shows the estimated Stokes-only upper bound heat transport by combining the depth-integrated Stokes transport with near surface temperature. 
		- Its zonal integration shows clear divergence from the storm-track regions and convergence towards the tropics, suggesting that the wave-induced pathway is spatially organised and may partly oppose the usual poleward redistribution of heat by large-scale circulations 
	- ~={red}gap: The question emerges: whether accounting this underrepresented wave-induced pathway in the ocean can leave a net and organised imprint on large-scale heat redistribution.=~
- **Introduce Stokes drift into ICON-o**
	- To move from potential transport to net climate-scale imprint, we introduce Stokes drift into the ICON-O in a dynamically consistent way. 
		- ==Figure 3 Mathematical expressions== shows the mathematical framework. Specifically, Stokes-induced momentum effects are included in the ICON-O primitive equations under the Wave Averaged Boussinesq framework
		- Once Stokes drift is included, the ocean adjust through compensating Eulerian currents, pressure-gradient changes, and altered vertical motion. These responses can oppose, reshape, or redistribute the impact of the imposed Stokes forcing.
		- The strength and vertical structure depend on regional conditions such as Coriolis strength and stratification. Therefore, the net imprint cannot be inferred from the diagnosed Stokes transport alone; We need a global ocean model that allows the imposed Stokes forcing and the ocean adjusts to interact dynamically
- **Current Status and Preliminary Diagnostics**
	- The Stokes-drift implementation in ICON-o is finished and verified
	- ==Show the details about the paired simulations==: paired reference/Stokes-forced simulations have started
		- prescribed Stokes drift velocity profile estimated from ERA5 wave reanalysis to force the ICON-O as the Stokes-forced simulation; 
...
- **Opening (~1 mins)**:
	- When we look at the ocean surface, the most visible motion is often the waves. These waves are generated by winds blowing over the ocean surface. 
	- As you step into the waves, you can feel that the wave itself is pushing you back and forth, nearly in a full orbital motion; However, it might also give you a net drift towards the wave propagating direction when there’s asymmetric orbital motion. This is the Stokes drift
	- Thus, since we human feels the Stokes drift, does the climate system also feel it?
	- ~={red}general gap: And since the Stokes drift also transport tracers like heat, salt; Does accounting this underrepresented wave-induced pathway in the ocean leave a imprint on the system?=~
- **Diagnostics from reanalysis data (~2 mins)**
  I first started with simple diagnostics from ERA5 wave reanalysis to estimate the scale and structure of the Stokes drift
	- ==Figure 2a== shows the climatological depth-integrated Stokes volume transport and its zonal integration. 
		- The map shows that the transport is spatially organised, with coherent equatorward components in the subtropics, away from major storm-track regions.
		- Zonal integration shows non-negligible amount of this Stokes volume transport
	- As Stokes drift also transport tracers like heat, I also estimate the Stokes-only heat transport by combining the depth-integrated Stokes transport with near surface temperature, exhibited in ==Figure 2b==
		- Its zonal integration shows clear divergence from the storm-track regions and convergence towards the tropics, suggesting that the wave-induced pathway is also spatially organised and may partly oppose the usual poleward redistribution of heat by large-scale circulations 
- **Introduce Stokes drift into ICON-o (~3 mins)**
  These diagnostics show scale and structure, but the net imprint depends on how the ocean responds dynamically.
	- To move from diagnosed potential transport to net climate-scale imprint, we introduce Stokes drift into the ICON-O in a dynamically consistent way. 
		- ==Figure 3== shows the mathematical framework. Specifically, Stokes-induced momentum effects are included in the ICON-O primitive equations under the Wave Averaged Boussinesq framework
		- Once Stokes drift is included, the ocean adjust through compensating Eulerian currents, pressure-gradient changes, and altered vertical motion. 
		- These responses can oppose, reshape, or redistribute the impact of the imposed Stokes forcing. Their strength and vertical structure depend on regional conditions such as Coriolis strength and stratification. 
		- **Therefore, the net imprint cannot be inferred from the diagnosed Stokes transport alone**; A global ocean model is needed to allow the imposed Stokes forcing and the ocean adjusts interact dynamically
...
### Draft 4
- **Opening (~1-2 mins)**:
	- When we look at the ocean surface, the most visible motion is often the waves. These waves are generated by winds blowing over the ocean surface. (Hasselmann; ==make a video of ERA5 swh or stokes?==)
	- As waves propagate, the asymmetric orbital motion of water parcels produces a small net drift in the direction of wave propagation. This is Stokes drift, and it can transport near-surface water, heat, salt, and other tracers.
	- This gives the motivation for my work: wind-induced waves may provide a previously under-quantified pathway of heat redistribution in the ocean.
	- ~={red}This leads to the central question of my work: Could wind-induced surface waves affect large-scale ocean heat redistribution through Stokes transport, a pathway that remains underrepresented in climate models?=~
- **Diagnostics from reanalysis data (~2 mins)**
	- ~={green}To make this question more concrete, I first used ERA5 wave reanalysis (using the wave model) to estimate the scale and spatial structure of Stokes drift and its depth-integrated transport=~
	- ==Figure 2a== shows the climatological depth-integrated Stokes volume transport and its zonal integration. 
		- The transport is spatially organised, with coherent equatorward components in the subtropics, away from major storm-track regions in both hemispheres.
		- Zonal integration further shows non-negligible amount of meridional Stokes volume transport, especially over the subtropics
	- As Stokes drift also transport tracers like heat, I also estimate the Stokes-only heat (temperature, how I defined?) transport by combining the depth-integrated Stokes transport with near surface temperature, exhibited in ==Figure 2b== (one part of the story)
		- Its zonal integration shows clear divergence from the storm-track regions and convergence towards the tropics, suggesting that the wave-induced pathway is also spatially organised and may partly oppose the usual poleward redistribution of heat by large-scale circulations
- **Introduce Stokes drift into ICON-o (~3 mins)**
	- ~={green}These reanalysis diagnostics show that this wave-induced transport is non-negligible and spatially organised, but the net imprint depends on how the ocean adjusts dynamically.=~
	- To move from diagnosed potential transport to net climate-scale imprint, we introduce Stokes drift into the ICON-O in a dynamically consistent way. 
		- ==Figure 3== shows the mathematical framework. Specifically, Stokes-induced momentum effects are included in the ICON-O primitive equations under the Wave Averaged Boussinesq framework
		- Once Stokes drift is included, the ocean adjust through compensating Eulerian currents, pressure-gradient changes, and altered vertical motion. 
		- These responses can oppose, reshape, or redistribute the impact of the imposed Stokes forcing. Their strength and vertical structure depend on regional conditions such as Coriolis strength and stratification. 
		- Therefore, the net imprint cannot be inferred from the diagnosed Stokes transport alone; A global ocean model is needed to allow the imposed Stokes forcing and the ocean adjusts interact dynamically
	- ~={red}With this implementation, the question becomes sharper: when Stokes drift is represented dynamically, does the resulting ocean adjustment leave a net, detectable, and organised imprint on circulation and heat redistribution?=~
- **Current Status and Preliminary Diagnostics (~2 mins)**
	- The Stokes-drift implementation in ICON-o is **finished** and verified (maybe not)
	- Paired simulations have started 
	- Preliminary diagnostics suggest that the oceanic adjustment extends below the shallow Stokes layer, consistent with a three-dimensional adjustment. Early heat-transport diagnostics show regional signals, including anomalous heat convergence in the Southern Ocean, which need to be tested with longer simulations.
- **Next step & Takeaway message (~1 mins)**
	- Next, quantify the depth structure, seasonal variability, regional organisation, and heat-transport signature of the residual response.  
...
> [!Important]
> - **The first question should be about whether this missing pathway is worth considering** (pathway-level).
> 	- “Could wind-induced surface waves affect large-scale ocean heat redistribution through Stokes transport, a pathway that remains underrepresented in climate models?”
> - **The second should be about what happens after we put it into the dynamical ocean model** (model-response level)
> 	- “when Stokes drift is represented dynamically, does the resulting ocean adjustment leave a net, non-negligible, and organised imprint on circulation and heat redistribution?”
